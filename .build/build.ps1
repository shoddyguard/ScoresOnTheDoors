<#
.SYNOPSIS
    Builds and releases the web application via Invoke-Build.
#>
[CmdletBinding()]
param
(
...
try
{
    ...
    {
        $BuildParams.Add('DockerHubUsername', $DockerHubUsername)
    }
    if ($DockerHubToken)
    {
        $BuildParams.Add('DockerHubToken', $DockerHubToken)
    }
    if ($GHCRToken)
    {
        $BuildParams.Add('GHCRToken', $GHCRToken)
    }
    if ($PublishTo)
    {
